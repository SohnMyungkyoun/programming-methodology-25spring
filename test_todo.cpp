...
TEST(TodoListTest, RemoveInvalidIndexThrows) {
    ...
    EXPECT_THROW(todo.remove_task(39), std::out_of_range);
...
TEST(TodoListTest, AddTooManyTasksThrows) {
    ...
    for (int i=0;i<32;i++){
        todo.add_task("Same thing every minuite");
    }
    EXPECT_THROW(todo.add_task("I want to do different things!"), std::out_of_range);
